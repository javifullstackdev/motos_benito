    ...
    <div className="max-w-6xl mx-auto px-4 py-8 text-neutral-200">
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-neutral-800 pb-6">
        ...
        <div className="flex items-center justify-between md:justify-end gap-3">
          ...
          <Link
            to="/items/new"
            className="inline-flex items-center gap-1.5 rounded-lg bg-orange-600 px-4 py-2 text-xs sm:text-sm font-semibold text-white shadow-lg shadow-orange-600/25 transition-all hover:bg-orange-500 active:scale-[0.99]"
          >
      ...
      <div className="mb-6 rounded-xl border border-neutral-800/90 bg-neutral-900/80 p-3 shadow-xl backdrop-blur-xl">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full rounded-lg border border-neutral-800 bg-neutral-950/90 py-2.5 pl-10 pr-4 text-xs sm:text-sm text-white placeholder-neutral-500 transition-all focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500"
          />
        </div>
      </div>
        ...
        {filteredItems.map((item) => (
          ...
          >
            ...
            <div className="mt-3 pt-3 border-t border-neutral-800/70">
              <Link
                to={`/items/${item.itemId}`}
                className="w-full flex items-center justify-center gap-1.5 rounded-xl bg-neutral-800/80 border border-neutral-700 py-2 text-xs font-bold text-neutral-200 hover:border-orange-500 hover:text-orange-400 transition-colors"
              >
              ...
              {filteredItems.map((item) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 font-mono font-medium text-orange-500">
                    <span className="rounded bg-neutral-950/80 px-2.5 py-1 border border-neutral-800 text-xs">
                      {Number(item.unitPrice).toFixed(2)} €
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    {item.inStock ? (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-950/50 border border-emerald-500/30 px-2.5 py-0.5 text-xs font-semibold text-emerald-400">
                        <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" />
                        En stock
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-neutral-950/80 border border-neutral-700 px-2.5 py-0.5 text-xs font-semibold text-neutral-400">
                        <span className="h-1.5 w-1.5 rounded-full bg-neutral-500" />
                        Bajo pedido / Agotado
                      </span>
                    )}
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link
                      to={`/items/${item.itemId}`}
                      className="inline-flex items-center rounded-md border border-neutral-700 bg-neutral-800/80 px-2.5 py-1 text-xs font-medium text-neutral-300 transition-colors hover:border-orange-500 hover:text-orange-400"
                    >
                      Editar / Detalle →
                    </Link>
                  </td>
